...
where
    for<'a> T: Mul + Clone + Sync + Send,
    for<'a> &'a T: Div<&'a T, Output = &'a T> + Mul<&'a T, Output = T>,
{
    if arr.is_empty() {
        vec![]
    } else {
        arr.par_iter().product().map_or(vec![], 
            |prod|
            arr.iter().map(|e| (prod / e)).collect()
        )
    }
}
    ...
    T: Clone,
{
    if arr.is_empty() {
        ...
        arr.iter().skip(1).for_each(|e| {
            left_prods.push(left_acc.clone());
            left_acc = &left_acc * e;
        });
        arr.iter().rev().skip(1).for_each(|e| {
            right_prods.push(right_acc.clone());
            right_acc = &right_acc * e;
        });
...
mod tests {
    ...
    fn without_div() {
        assert_eq!(
            vec![120., 60., 40., 30., 24.,],
            prod_from_elems_without_div(&[1., 2., 3., 4., 5.,])
        )
    }
