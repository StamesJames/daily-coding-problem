...
pub fn find_palindrome_pairs_brute_force(words: &[&str]) -> Vec<(usize, usize)> {
    let all_pairs = words
        .iter()
        .enumerate()
        .flat_map(
...
pub fn find_palindrome_pairs_with_map(words: &[&str]) -> Vec<(usize, usize)> {
    let mut result = Vec::new();
    let mut word_index_map = HashMap::new();
    words.iter().enumerate().for_each(|(i, elem)| {
        word_index_map.insert(*elem, i);
    });
...
mod tests {
    ...
    fn test_code_edoc_da_d_brute_force() {
        ...
        assert_eq!(
            solution,
            find_palindrome_pairs_brute_force(&example).as_slice()
        );
    }
    ...
    fn test_code_edoc_da_d_with_map() {
        ...
        assert_eq!(
            solution,
            find_palindrome_pairs_with_map(&example).as_slice()
        );
    }
}
