...
pub fn calculate_max_sub_array_sum_wrap<T>(arr: &[T]) -> T
where
    ...
    T: Clone + Zero,
{
    ...
    for elem in arr.iter().chain(arr.iter()) {
        let new_max_last_elem = if &(&max_last_elem + elem) > elem {
            count_max_last_elem += 1;
            &max_last_elem + elem
        } else {
            count_max_last_elem = 1;
            elem.clone()
        };
        let new_max = if &new_max_last_elem > &max {
            new_max_last_elem.clone()
        } else {
            max
        };
...
pub fn calculate_max_sub_array_sum<T>(arr: &[T]) -> T
where
    for<'a> &'a T: Add<&'a T, Output = T> + Ord,
    T: Clone + Zero,
{
...
mod tests {
    ...
    fn without_wrap() {
        assert_eq!(
            7,
            calculate_max_sub_array_sum(&[1, 1, 1, -5, 1, 1, 1, -5, 1, 1, 1, 1, 1, 1, 1])
        )
    }
    ...
    fn with_wrap() {
        assert_eq!(
            10,
            calculate_max_sub_array_sum_wrap(&[1, 1, 1, -5, 1, 1, 1, -5, 1, 1, 1, 1, 1, 1, 1])
        )
    }
}
